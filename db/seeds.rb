...
TariffDuration.create(tariff: tariff_day, start_point: 1, end_point: 2, price: 40.0)
TariffDuration.create(tariff: tariff_day, start_point: 2, end_point: 3, price: 30.0)
...
(1..25).map { Bike.generate_identifier }.each_with_index do |i, k|
  Bike.create(identifier: i, bike_type: rand(1..4), address: Address.all.sample, start_date: k.days.ago)
end

[:admin, :client].each do |u_type|
  User.create(
